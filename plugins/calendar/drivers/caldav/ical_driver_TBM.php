...
class ical_driver extends calendar_driver
{
    ...
    private $calendar_sync_clients = array();
    ...
    public function calendar_form($action, $calendar, $formfields)
    {
        ...
        $input_caldav_url = new html_inputfield(array("name" => "caldav_url", "id" => "caldav_url", "size" => 20));
        ...
        $input_caldav_user = new html_inputfield(array("name" => "caldav_user", "id" => "caldav_user", "size" => 20));
        ...
        $input_caldav_pass = new html_passwordfield(array("name" => "caldav_pass", "id" => "caldav_pass", "size" => 20));
        $formfields["caldav_pass"] = array(
            "label" => $this->cal->gettext("password"),
            "value" => $input_caldav_pass->show($calendar["caldav_pass"]),
            "id" => "caldav_pass",
        );
        ...
        if (!empty($this->rc->user->ID)) {
         ...
         ORDER BY name",
            ...
            while ($result && ($arr = $this->rc->db->fetch_assoc($result))) {
                ...
                $this->calendar_sync_clients[$arr['calendar_id']] =
                    new ical_sync($this->cal, $arr);
     ...
     *  caldav_url: URL to external iCal/caldav calendar
     * caldav_user: Optional username if auth is required.
     * caldav_pass: Optional password if auth is required.
     * caldav_ctag: Optional caldav ctag.
    ...
    public function create_calendar($prop)
    {
       ...
       (user_id, name, color, showalarms, caldav_url, caldav_user, caldav_pass)
       VALUES (?, ?, ?, ?, ?, ?, ?)",
            $this->rc->user->ID,
            $prop['name'],
            $prop['color'],
            ...
            $prop['caldav_url'],
            $prop['caldav_user'],
            $prop['caldav_pass'], // TODO: Encrypt password e.g. with user's real password.
            $prop['caldav_ctag']
        );
    ...
    public function edit_calendar($prop)
    {
       ...
       SET   name=?, color=?, showalarms=?, caldav_url=?, caldav_user=?, caldav_pass=?, caldav_ctag=?
       WHERE calendar_id=?
       AND   user_id=?",
            $prop['name'],
            $prop['color'],
            ...
            $prop['caldav_pass'] ? $prop['caldav_pass'] : null, // TODO: Encrypt password e.g. with user's real password.
            $prop['caldav_ctag'] ? $prop['caldav_ctag'] : null,
            $prop['id'],
            $this->rc->user->ID
        );
    ...
    public function new_event($event)
    {
        ...
        if (!empty($this->calendars)) {
         ...
         (calendar_id, created, changed, uid, %s, %s, all_day, recurrence, title, description, location, categories, url, free_busy, priority, sensitivity, attendees, alarms, notifyat, caldav_etag, caldav_url)
         VALUES (?, %s, %s, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    public function edit_event($event)
    {
        if (!empty($this->calendars)) {
            ...
            // Seems that $event is ok for caldav here!
    ...
    private function _update_event($event, $update_recurring = true)
    {
        ...
        $sql_set = array();
        $set_cols = array('start', 'end', 'all_day', 'recurrence_id', 'sequence', 'title', 'description', 'location', 'categories', 'url', 'free_busy', 'priority', 'sensitivity', 'attendees', 'alarms', 'notifyat', 'caldav_etag', 'caldav_url');
       ...
       AND   calendar_id IN (" . $this->calendar_ids . ")",
        ...
        // TODO: Move this to edit_event() ???
        // Sync event to caldav. We do this after changes were made to the db, since this way we can
        // easily get the event from db and use libvcalender::export() to create the ical format.
        if ($success) {
            $event = $this->calendar_sync_clients[$event["calendar"]]->update_event($this->get_event($event));

            // TODO: Update etag and url.
            // TODO: Make sure db constraints allow storing events without etag/url.
        }
    ...
    /**
     * Synchronizes events and gets recent event data.
     *
     * @see calendar_driver::load_events()
     */
    public function load_events($start, $end, $query = null, $cal_ids = null)
    {
        foreach ($this->calendar_sync_clients as $cal_id => $cal_sync) {
            if (!$cal_sync->is_synced()) {
                $this->cal->debug_log("Syncing calendar id \"$cal_id\".");
                $events = $this->_load_db_events(null, null, null, array($cal_id));
                $updates = $cal_sync->get_updates($events);
                foreach ($updates as $update) {
                    if (isset($update["event"])) {
                        if ($update["action"] == ical_sync::ACTION_CREATE) {
                            $event_id = $this->new_event($update["event"]);
                        } elseif ($update["action"] == ical_sync::ACTION_UPDATE) {
                            $this->edit_event($update["event"]);
                        } elseif ($update["action"] == ical_sync::ACTION_DELETE) {
                            $this->remove_event($update["event"]);
                        }
                    } else if (isset($update["calendar"])) {
                        if ($update["action"] == ical_sync::ACTION_UPDATE) {
                            $cal = $update["calendar"];
                            $this->calendars[$cal["id"]] = $cal;
                            $this->edit_calendar($cal);
                        }
                    }
                }
            }
        ...
        return $this->_load_db_events($start, $end, $query, $cal_ids);
    }

    /**
     * Get event data from database.
     *
     * @param int Start unixtimestamp for event date/time range.
     * @param int End unixtimestamp for event date/time range.
     * @param string Query expression for event columns title, location, description
     *        categories and attendees.
     * @param array List or coma separated string of calendar id's to load events from.
     * @return List of loaded events.
     */
    private function _load_db_events($start = null, $end = null, $query = null, $cal_ids = null)
    {
        if (empty($cal_ids))
            $cal_ids = array_keys($this->calendars);
        else if (is_string($cal_ids))
            $cal_ids = explode(',', $cal_ids);

        // only allow to select from calendars of this use
        $cal_ids = array_map(array($this->rc->db, 'quote'),
            array_intersect($cal_ids, array_keys($this->calendars)));
        ...
        $date_query = "";
        if ($start && $end) {
            $date_query = sprintf("AND e.start <= %s AND e.end >= %s",
        ...
        if (!empty($cal_ids)) {
         ...
         GROUP BY e.event_id",
            ...
            $result = $this->rc->db->query($sql_query);
